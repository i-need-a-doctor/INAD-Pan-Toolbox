...
def GSA(pan, hs):
    x_hrpan = einops.rearrange(pan, "c h w -> w h c")
    x_lrms = einops.rearrange(hs, "c h w -> w h c")
    M, N, c = x_hrpan.shape
    m, n, C = x_lrms.shape
    ...
    # upsample
    u_hs = upsample_interp23(x_lrms, ratio)
    ...
    image_lr_lp = x_lrms - np.mean(x_lrms, axis=(0, 1))

    # sintetic intensity
    image_hr = x_hrpan - np.mean(x_hrpan)
